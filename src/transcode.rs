...
pub fn transcode(input: &Path, output: &Path) -> Result<()> {
    ...
    let pixel_bytes = decoded.to_vec().context("Failed to convert decoded pixels to vector")?;
    ...
    new_obj.put(DataElement::new(pixel_data_tag, vr, PrimitiveValue::from(pixel_bytes)));
    ...
    let sop_class_uid = new_obj.element(Tag(0x0008, 0x0016))
        .ok()
        .and_then(|e| e.to_str().ok())
        .unwrap_or(Cow::Borrowed("1.2.840.10008.5.1.4.1.1.7"));

    let sop_instance_uid = new_obj.element(Tag(0x0008, 0x0018))
        .ok()
        .and_then(|e| e.to_str().ok())
    ...
    file_obj.write_to_file(output).context("Failed to write output file")?;
